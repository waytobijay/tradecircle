 ...
 * Authenticated shell for Seller role.
 * Spec ref: section 2 (Navigation — Seller)
 *
 * Navbar:
 *  - Left:   Logo
 *  - Centre: Home | My Products | List Product | Messages
 *  - Right:  Notification bell + Profile dropdown
 * Dropdown: Profile / Analytics / Settings / Logout
 * Bottom: MobileNav
 */
...
import {
  Bell, ChevronDown,
  User, BarChart2, Settings, LogOut, Package, Plus,
} from 'lucide-react';
...
const NAV_LINKS = [
  ...
  { href: '/products/new',  label: 'List Product'  },
...
export default function SellerLayout({ children }: SellerLayoutProps) {
  ...
  return (
    <>
      <style>{`
        @media (max-width: 767px) { .sl-desktop { display: none !important; } }
        @media (min-width: 768px) { .sl-desktop { display: flex !important; } }
      `}</style>
      ...
      <header style={{
        ...
        <div style={{
          ...
          <Link href="/home" style={{ textDecoration: 'none', flexShrink: 0 }}>
            <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-seller, #f59e0b)', letterSpacing: '-0.02em' }}>
              TradeCircle
            </span>
          </Link>

          <nav className="sl-desktop" style={{ gap: 28 }}>
            {NAV_LINKS.map((l) => (
              <Link key={l.href} href={l.href} style={{
                textDecoration: 'none', fontSize: 14,
                fontWeight: pathname.startsWith(l.href) ? 600 : 400,
                color: pathname.startsWith(l.href) ? 'var(--color-seller, #f59e0b)' : 'var(--color-text)',
                transition: 'color 0.15s',
                display: 'flex', alignItems: 'center', gap: 4,
              }}>
                {l.href === '/products/new' && <Plus size={14} />}
                {l.label}
              </Link>
            ))}
          </nav>
              ...
              >
                {user?.profilePhoto
                  ...
                  : <div style={{
                      ...
                      background: 'var(--color-seller, #f59e0b)', color: '#fff',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 13, fontWeight: 600,
                    }}>{initials}</div>
                }
                <ChevronDown size={14} color="var(--color-text-secondary)" className="sl-desktop" style={{ display: 'none' }} />
              </button>
              ...
              {dropdownOpen && (
                <div style={{
                  ...
                  {[
                    ...
                    { href: '/my-products',             icon: <Package size={15} />,  label: 'My Listings'},
                    { href: '/seller/analytics',        icon: <BarChart2 size={15} />,label: 'Analytics' },
